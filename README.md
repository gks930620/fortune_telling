...
매일(daily) · 월요일(weekly) · 매월 1일(monthly) · 1월 1일(yearly) · 연휴 첫날(holiday 특집)
...
- 날짜·일진(60갑자)·십성·연휴 판정은 **코드**(`scripts/`), 해석·글은 **AI**(`prompts/`)
...
FORTUNE_DATE=2026-01-01 node scripts/context.js  # 특정 날짜 재생(신년·연휴 테스트)
node scripts/assemble.js                       # work/<날짜>/*.json 검증·병합
```

## 정기 유지보수 (연 1회)

- 매년 12월: `scripts/holidays.js`에 다음 해 공휴일 추가 (없으면 경고 로그만 뜨고 연휴 특집만 빠짐)
